...
public class Http {
    ...
    */
    private func request(url: String, parameters: [String: AnyObject]? = nil,  method: HttpMethod,  credential: NSURLCredential? = nil, completionHandler: CompletionBlock) {
        ...
        if (self.authzModule != nil) {
            ...
            else if ClientService.sharedInstance.validateToken() {
                block()
            }
            else { // check expire time - if within 5 minutes or after - try refresh token
                if let refreshToken = NSUserDefaults.standardUserDefaults().valueForKey(Global.API_REFRESH_TOKEN) as? String {
                    if !isRefreshing {
                        ...
                        ClientService.sharedInstance.refreshToken(refreshToken, completion: { (success) -> () in
                            isRefreshing = false
                            if success == nil {
                                completionHandler(nil, NSError(domain: "Refresh Token is invalid.", code: 400, userInfo: nil))
                                return
                            }
                            block()
                        })
    ...
    */
    ...
    public func LOGIN(url: String, parameters: [String: AnyObject]? = nil, credential: NSURLCredential? = nil, completionHandler: CompletionBlock) {
        request(url, parameters: parameters, method:.LOGIN, credential: credential, completionHandler: completionHandler)
    }
